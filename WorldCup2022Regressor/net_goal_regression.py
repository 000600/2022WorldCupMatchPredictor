...
for game in df_list:
  # Input is a list: [first country's FIFA ranking, second country's FIFA ranking]
  x.append([game[2] - game[3]])

  # Output is the net score (first country's score - second country's score)
...
# Output layer
model.add(Dense(1)) # No activation function because the model is a regression algorithm
...
print(f"\nModel's Prediction: {prediction} | Actual Results: {actual_value}")
